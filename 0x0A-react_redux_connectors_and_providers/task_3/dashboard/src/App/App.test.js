...
import { App, listNotificationsBase, mapStateToProps } from "./App";
...
describe("<App />", () => {
  ...
  it("verify that markNotificationAsRead works as intended", () => {
    ...
    expect(wrapper.state().listNotifications).toEqual(listNotificationsBase);
    ...
    expect(wrapper.state().listNotifications).toEqual(listNotificationsBase.slice(0, 2));

    instance.markNotificationAsRead(1);

    expect(wrapper.state().listNotifications).toEqual(listNotificationsBase.slice(1, 2));
  });
});
